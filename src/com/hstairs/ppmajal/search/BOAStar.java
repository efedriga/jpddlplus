...
public class BOAStar extends SearchEngine{
    ...
    public SimpleSearchNode search(SearchProblem problem, SearchHeuristic heuristic, PrintStream out) {
		...
		final ObjectHeapPriorityQueue<BoaStarSearchNode> frontier = new ObjectHeapPriorityQueue<>(new BoaStarTieBreaker());
		...
		for (float[] hVector : hAtInit) {
			if (hVector[0] == Float.MAX_VALUE || hVector[1] == Float.MAX_VALUE) continue;
		...
		while(!frontier.isEmpty()){
			...
			for (final Iterator<Pair<State, Object>> it = problem.getSuccessors(currentNode.s, actionsToSearch); it.hasNext();) {
				...
				for (float[] hVector : heuristics) {
					...
					BoaStarSearchNode node = new BoaStarSearchNode(successorState, act, currentNode, successorG1, successorG2, successorF1, successorF2);
	...
	private boolean isDeadEnd(List<float[]> heuristics) {
        if (heuristics == null || heuristics.isEmpty()) return true;
        for (float[] hVector : heuristics) {
